...
export const createPopupTemplate = (film) => {
  ...
  const createFilmGenresTemplate = (arr) => {
    let genresListTemplate = ``;
    for (let i = 0; i < arr.length; i++) {
      genresListTemplate = genresListTemplate.concat(`<span class="film-details__genre">${arr[i]}</span>`);
    }
  ...
  const createFilmCommentsTemplate = (arr) => {
    let commentsListTemplate = ``;
    for (let i = 0; i < arr.length; i++) {
      commentsListTemplate = commentsListTemplate.concat(commentTemplate(arr[i]));
    }
